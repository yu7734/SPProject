...
public class JustDodgeManager : MonoBehaviour
{
    ...
    [SerializeField, Tooltip("EXPポップアップの出現位置オフセット（JUST!の下に出すなら 30 など）")] private Vector2 expPopupScreenOffset = new Vector2(0f, 30f);
    ...
    void Start()
    {
        // JUST!テキストの初期状態を保存して、最初は非表示にしておく
        if (justText != null)
        {
    ...
    void Update()
    {
        ...
        if (isShowingJustText && justText != null)
        {
            ...
            {
                // 終了 → GameObject ごと非表示にしてアイテム選択UIなどのRaycastを邪魔しないようにする
                alpha = 0f;
                scale = 1f;
    ...
    {
        if (other.CompareTag(Tags.EnemyBullet) && (player.bJustDodge == true))
        {
    ...
    /// <summary>
    /// プレイヤーのワールド座標を画面座標に変換して、JUST!テキストの位置を更新する。
